...
//***************************
    ...
    $rootScope.sectionClick = function(sectionId) {
      if ( !$rootScope.user.getSection(sectionId).getIsVisited() ) return; //not clickable, so return
      var isSameSection = ($rootScope.user.currentLocation.sectionId === sectionId);
      $rootScope.user.hideSectionInner = (isSameSection ? !$rootScope.user.hideSectionInner : false); //toggle section inner visibility
      $rootScope.setLocation(sectionId, undefined, false);
    };
    ...
    $rootScope.setLocation = function(sectionId, pageId, allowBeyondFurthestLocation) {
      ...
      if (userService.setLocation(sectionId, pageId, allowBeyondFurthestLocation)) {
        if (sectionId === 0) { //treat section 0 differently
          if (pageId === 1) { window.location = 'http://localhost/th/4/profile/profile_1.html'; }
          if (pageId === 2) { window.location = 'http://localhost/th/4/settings/settings.html'; }
        } else {
          if (pageId === undefined) pageId = 1; //default to 1
  ...
  controller('summaryController', function($scope, $location, $route, userService) {
    $scope.getStyle = function(sectionId) { //returns the height of a current section, o/w nothing
      ...
      if (!section.getIsCurrent()) return;
      if ($scope.user.hideSectionInner) return;
    ...
    $scope.getClass = function(sectionId, pageId, typeId) {
      ...
      if (category == 'section') {
        if (typeId == 1) { //section (outer)
          ...
          s += (section.getIsCurrent() && !$scope.user.hideSectionInner ? ' sectionInner-isCurrent' : ' sectionInner-notCurrent');
        };
